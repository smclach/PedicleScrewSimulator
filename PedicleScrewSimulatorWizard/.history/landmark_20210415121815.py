...
class LandmarksStep( PedicleScrewSimulatorStep ):
    ...
    def updateTable(self):
      ...
      self.fid = self.fiducialNode()
        self.fids = self.fid.GetNumberOfFiducials()
        logging.debug(self.fids)
        self.fidLabels = []
        self.items = []
        self.Label = qt.QTableWidgetItem()

        self.table2.setRowCount(self.fids // 2)

        for i in range(0, self.fids // 2):
            self.Label = qt.QTableWidgetItem(
            ...
            self.combofidlist.addItems(self.screwAng)
    ...
    def deleteFiducial(self):
        if self.table2.currentColumn() == 0:
            ...
            self.fids = self.fidNode.GetNumberOfFiducials()
            
            for i in range(0, self.fids):
    ...
    def addFiducialToTable(self, observer, event):
        ...
        self.table2.setRowCount(self.fids)
    ...
    def onEntry(self, comingFrom, transitionType):
        ...
        for i in range(self.levels.index(self.vertebra),
            ...
            if self.vertebra == "部位":
                self.screwAng.append(self.vertebra + "_" + str(i) + "/--/入皮点")
                self.screwAng.append(self.vertebra + "_" + str(i) + "/--/靶点")
            else:
                if self.sides == "左右":
                    self.screwAng.append(self.levels[i] + "/左" + "/最窄点")
                    # self.screwAng.append(self.levels[i] + "/-" + "/椎前点")
                    self.screwAng.append(self.levels[i] + "/右" + "/最窄点")
                    self.screwAng.append(self.levels[i] + "/-" + "/椎前点")

                else:
                    self.screwAng.append(self.levels[i] + "/" + self.sides + "/入皮点")
                    self.screwAng.append(self.levels[i] + "/" + self.sides + "/靶点")
        logging.debug("Fiducial list: {0}".format(self.screwAng))
